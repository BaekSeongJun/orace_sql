...
SELECT sm.user_no, sm.user_id, sm.user_pw ,sma.auth
FROM securitymember sm
INNER JOIN securitymember_auth sma ON sm.user_no = sma.user_no
WHERE sm.user_no >= 2;

select * from securitymember;

SELECT mem.user_no, mem.user_id, user_pw, user_name, enabled, reg_date, upd_date, auth
        FROM securitymember mem INNER JOIN securitymember_auth auth ON
            mem.user_no = auth.user_no
        WHERE mem.user_id = 'admin4';
